...
=======
                <input type="text" :placeholder="$root.$t('Write a message') + '...'"
                    class="w-full p-3 outline-none focus:ring-1 focus:ring-primary" required v-model="message"
                    :disabled="!enabledSendButton || loading" :class="masterStore.langDirection == 'rtl' ? 'pl-12' : 'pr-12'">
                <button v-if="enabledSendButton" class="send-btn bg-primary btn" type="submit" :class="masterStore.langDirection == 'rtl' ? 'left-[6px]' : 'right-[6px]'">
                    <PaperAirplaneIcon v-if="!loading" class="w-5 h-5 -rotate-45" />
                    <LoadingSpin v-else color="text-white" />
...
=======
const emits = defineEmits(['messageSubmitted']);
>>>>>>> aa6c636e1 (أول رفع لموقع wepower.host)
